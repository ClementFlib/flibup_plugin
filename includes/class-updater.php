...
class Updater
{
	...
	public function register_hooks()
	{
		add_filter(
			...
			array($this, 'inject_update')
		);

		add_filter(
			'plugins_api',
			array($this, 'plugin_details'),
			10,
			3
		);
		...
		add_filter(
			...
			array($this, 'fix_source_dir'),
			10,
			4
		);

		/*
	 * Purge le cache GitHub après l'installation d'une mise à jour.
	 */
		add_action(
		...
		/*
	 * Ajoute un lien de vérification manuelle sous le plugin.
	 */
		add_filter(
			'plugin_action_links_' . $this->basename,
			array($this, 'add_manual_check_link')
		);

		/*
	 * Traite la demande de vérification manuelle.
	 */
		add_action(
			'admin_post_flibup_check_updates',
			array($this, 'handle_manual_check')
		);
	}
	...
	/**
	 * Ajoute un lien « Vérifier les mises à jour » sous Flib'Up.
	 *
	 * @param array $links Liens d'action du plugin.
	 * @return array
	 */
	public function add_manual_check_link($links)
	{
		...
		$url = wp_nonce_url(
			admin_url('admin-post.php?action=flibup_check_updates'),
			'flibup_check_updates'
		);

		$manual_link = sprintf(
			'<a href="%s">%s</a>',
			esc_url($url),
			esc_html__('Vérifier les mises à jour', 'flib-up')
		);
	...
	/**
	 * Vide les caches et force une nouvelle vérification des mises à jour.
	 *
	 * @return void
	 */
	public function handle_manual_check()
	{
		...
		check_admin_referer('flibup_check_updates');

		/*
	 * Cache propre à l'appel GitHub de Flib'Up.
	 */
		delete_transient(self::TRANSIENT);

		/*
	 * Cache natif utilisé par WordPress pour les extensions.
	 */
		delete_site_transient('update_plugins');

		/*
	 * Force immédiatement une nouvelle vérification.
	 * Cela ne lance aucune installation automatique.
	 */
		wp_update_plugins();

		wp_safe_redirect(
			add_query_arg(
				'flibup_update_checked',
				'1',
